...
type cbirrtTimeOptions struct {
	...
	// This is how far cbirrtTime will try to extend the map towards a goal per-step. Determined from FrameStep
	qstep []float64
...
var zeroInputs = []referenceframe.Input{referenceframe.Input{0},referenceframe.Input{0},referenceframe.Input{0}}


// newCbirrtTimeOptions creates a struct controlling the running of a single invocation of cbirrtTime. All values are pre-set to reasonable
// defaults, but can be tweaked if needed.
func newCbirrtTimeOptions(planOpts *plannerOptions, frame referenceframe.Frame) (*cbirrtTimeOptions, error) {
	algOpts := &cbirrtTimeOptions{
...
// cBiRRTTimeMotionPlanner an object able to solve constrained paths around obstacles to some goal for a given referenceframe.
...
type cBiRRTTimeMotionPlanner struct {
	*planner
	tf referenceframe.TimeFrame
	fastGradDescent *NloptIK
	algOpts         *cbirrtTimeOptions
	corners         map[node]bool
}

// newCBiRRTTimeMotionPlannerWithSeed creates a cBiRRTTimeMotionPlanner object with a user specified random seed.
func newCBiRRTTimeMotionPlanner(
	frame *referenceframe.TimeFrame,
	seed *rand.Rand,
	logger golog.Logger,
	opt *plannerOptions,
) (motionPlanner, error) {
	...
	algOpts, err := newCbirrtTimeOptions(opt, mp.frame)
	if err != nil {
		return nil, err
	}
	return &cBiRRTTimeMotionPlanner{
		planner:         mp,
...
func (mp *cBiRRTTimeMotionPlanner) plan(ctx context.Context,
	...
	seedPose, err := mp.tf.Transform(seed)
	if err != nil {
		return nil, err
	}
	startFrame := mp.tf.AtNewPose(seedPose)
	endFrame := mp.tf.AtNewPose(spatialmath.Compose(seedPose, goal))
	...
	seedNode := &frameNode{q: seed, tf: startFrame}
	goalNode := &frameNode{q: zeroInputs, tf: endFrame}
	rrt.maps.startMap[seedNode] = nil
	rrt.maps.goalMap[goalNode] = nil
...
func (mp *cBiRRTTimeMotionPlanner) rrtBackgroundRunner(
	ctx context.Context,
	seed []referenceframe.Input,
	...
	target := referenceframe.InterpolateTimeInputs(seed, rrt.maps.optNode.Q(), 0.5)
	...
	mp.logger.Debugf(
		"running CBiRRTTime from start pose %v with start map of size %d and goal map of size %d",
	...
	for i := 0; i < mp.algOpts.PlanIter; i++ {
		select {
		case <-ctx.Done():
			mp.logger.Debugf("CBiRRTTime timed out after %d iterations", i)
			rrt.solutionChan <- &rrtPlanReturn{planerr: fmt.Errorf("cbirrtTime timeout %w", ctx.Err()), maps: rrt.maps}
			return
		default:
		}
		...
		// Solved!
		if reachedDelta <= mp.algOpts.JointSolveDist {
			mp.logger.Debugf("CBiRRTTime found solution after %d iterations", i)
			cancel()
...
func (mp *cBiRRTTimeMotionPlanner) sample(rSeed node, sampleNum int) []referenceframe.Input {
...
func (mp *cBiRRTTimeMotionPlanner) constrainedExtend(
	ctx context.Context,
	randseed *rand.Rand,
...
func (mp *cBiRRTTimeMotionPlanner) constrainNear(
	ctx context.Context,
	randseed *rand.Rand,
	seedInputs,
...
func (mp *cBiRRTTimeMotionPlanner) smoothPath(
	ctx context.Context,
	inputSteps []node,
) []node {
	...
	for iter := 0; iter < toIter && len(inputSteps) > 4; iter++ {
		select {
		case <-ctx.Done():
			mp.logger.Debug("CBiRRTTime timed out during smoothing, returning best path")
			return inputSteps
		default:
		}
...
func getTimeFrameSteps(f referenceframe.Frame, by float64) []float64 {
	dof := f.DoF()
