...
func newDualQuaternionFromRotation(o Orientation) Pose {
...
func newDualQuaternionFromDH(a, d, alpha float64) Pose {
	m := mgl64.Ident4()
...
// protobuf pose.
func newDualQuaternionFromProtobuf(pos *commonpb.Pose) Pose {
...
// new one.
//~ func newDualQuaternionFromPose(p Pose) Pose {
	//~ if q, ok := p.(Pose); ok {
		//~ return q.Clone()
	//~ }
...
//~ // newDualQuaternionFromPose takes any pose, checks if it is already a DQ and returns that if so, otherwise creates a
//~ // new one.
//~ func dualQuaternionFromPose(p Pose) Pose {
	//~ if q, ok := p.(Pose); ok {
		//~ return q
	//~ }
...
func (q *Pose) ToProtobuf() *commonpb.Pose {
	final := &commonpb.Pose{}
...
func (q *Pose) Invert() Pose {
	return Pose{dualquat.ConjQuat(q.Number)}
}

// SetZ sets the z translation.
func (q *Pose) SetZ(z float64) {
	q.Dual.Kmag = z
}

// Transformation multiplies the dual quat contained in this dualQuaternion by another dual quat.
func (q *Pose) Transformation(by dualquat.Number) dualquat.Number {
...
func OffsetBy(a, b *commonpb.Pose) *commonpb.Pose {
	...
	q3 := Pose{q1.Transformation(q2.Number)}
